function [ACPower, Pdc, mSAPMResults] = pvOutputCoords(P, latitude, longitude)
%pvOutputCoords Calculates PV output for a specified array of rated power,
...
% Define the Inverter
load([pwd,'\Third Party\PV_LIB Version 1_32 Release\Required Data\SandiaInverterDatabaseSAM2014.1.14.mat']);
...
Array.Ms=ceil(Inverter.Vdc0/(ModuleParameters.Voc0*(1+((ModuleParameters.BetaVoc*1e-2)*(Tm-25)))));
Pmm=ModuleParameters.Vmp0*ModuleParameters.Imp0;
Array.Mp=ceil(P/(Pmm*Array.Ms));
...
% t2=datetime(2014,12,31,23,30,0);
% t=t1:minutes(30):t2;
% TimeMatlab=(datenum(t))';
% Time = pvl_maketimestruct(TimeMatlab, ones(size(TimeMatlab))*0);
